...
olybasket <- filter(olympics, sport == 'Basketball')
...
olympics %>%
  ...
  ggplot(aes(x=year, y = mean_age, group = sex, color = sex)) +
  geom_line()
```
...
oly_numb %>%
  ggplot(aes(x = index, y = cumsum(medals), color = id)) +
  geom_line(alpha = 0.1, group = id) +
  geom_point()
```
