...
To answer our first question, we will evaluate the variables `height`, `weight`, `age`, `sex`, `sport`, and `year`. After looking at the dataset, we were interested in exploring the physical characteristics of Olympic athletes. We are all used to hearing that these athletes are in peak physical condition, but this classification differs by sport and has changed over the years.

We will start by creating some new variables, the first being `height-weight-ratio`. We are interested in exploring the height to weight ratio for different sports in the Olympics, as different shows require different body-types for success. For example, volleyball and basketball players are usually exceptionally tall, while wrestlers are usually shorter and built. We will create this variable and some plots to see if we can recognize any interesting info. We will also create other exploratory variables like `height-weight-ratio-by-age` to see if this ratio differs by age.

Finally, after creating these variables, we want to see how these ratios and other characteristics, like height and weight, differ in different sports. We can accomplish this by faceting by sport. To see how these values have changed over the years, we will use `year` to make time series plots for these different values.
