...
olympics_gdp %>%
  ...
  geom_text(data = highlight, aes(x = gdp_per_capita, y = total_winners, label = flag)) +
  facet_grid(~year, ) +
  labs(title = "",
       y = "Total Medal Winners",
...
``` r
highlight <- olympics_gdp %>%
  filter(!is.na(gdp_per_capita), season == "Summer", year %in% c(2002:2020), total_winners > 100)

olympics_gdp %>%
  filter(!is.na(gdp_per_capita), season == "Summer", year %in% c(2002:2020)) %>%
  ggplot(aes(x = gdp_per_capita, y = total_winners)) +
  geom_point() +
  geom_text(data = highlight, aes(x = gdp_per_capita, y = total_winners, label = entity)) +
  facet_wrap(~year)
```

<img src="README_files/figure-gfm/unnamed-chunk-10-2.png" width="80%" />

animate(p, duration = 20, fps = 10, renderer = gifski\_renderer())

``` r
p <- olympics_gdp %>%
  filter(!is.na(gdp_per_capita), season == "Winter", year > 1990) %>%
  ggplot(aes(x = gdp_per_capita, y = total_score)) +
...
animate(p, duration = 20, fps = 10, renderer = gifski_renderer())
```

<img src="README_files/figure-gfm/unnamed-chunk-11-1.gif" width="80%" />
